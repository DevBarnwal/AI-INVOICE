...
const Navbar = () => {
    ...
    // for token generation and storage in local storage(meaning fetch and store also refresh for if not found)
    ...
    //keep the local storage token in sync with clerk auth state
    useEffect(() => {
        let mounted = true;
    ...
    //after sucessful login redirect us to dashboard
    useEffect(() => {
        if(isSignedIn){
