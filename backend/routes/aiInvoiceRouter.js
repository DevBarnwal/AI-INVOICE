...
if(!API_KEY){
    console.log("No Gemini key found in the .env file here");
}
...
// in the prompt you can provide these details and it can fill them automatically in invoice
...
aiInvoiceRouter.post('/generate', async (req, res) => {
    try {
        ...
        if (!lastText) {
            const errMsg = (lastErr && lastErr.message) || "All candidate models failed. Check API key, network, or model availability.";
        ...
        try {
            ...
            console.log('Failed to parse json data from AI response', parseErr, { model: usedModel, jsonText });
            return res.status(502).json({
                success: false,
        ...
        return res.status(500).json({ success: false, message: "AI generation failed", detail: err?.message || String(err) });
    }
});
